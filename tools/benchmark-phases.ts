...
// Wrap the post-trunk stages to see how much of the run happens outside the
// Evoformer blocks the dispatch profiler covers.
const phases = new Map<string, { milliseconds: number; calls: number }>();
function timed<T extends { prototype: { run: (...args: never[]) => Promise<unknown> } }>(
  target: T, name: string,
): void {
  const original = target.prototype.run;
  target.prototype.run = async function patched(this: unknown, ...args: never[]) {
    const start = performance.now();
    try { return await original.apply(this, args); }
    finally {
      const entry = phases.get(name) ?? { milliseconds: 0, calls: 0 };
      entry.milliseconds += performance.now() - start; entry.calls += 1;
      phases.set(name, entry);
    }
  } as typeof original;
}
timed(StructureModuleGpu as never, "structure-module");
timed(ConfidenceHeadsGpu as never, "confidence-heads");
...
try {
  ...
  for (const [name, entry] of phases) {
    console.error(`${name}: ${entry.milliseconds.toFixed(0)} ms over ${entry.calls} calls`);
  }
} finally {
  device.destroy();
}
