...
describe("binding shards", () => {
  ...
  it("declares one binding a shard and reads through the right one", () => {
    const layout = planShards(400, 100, 800, 4);
    expect(layout).toEqual({ count: 2, shardElements: 200, totalElements: 400 });
    ...
    expect(loader).toContain("const PAIR_SHARD: u32 = 200u;");
  ...
  it("stores by element when exact and by word when packed", () => {
    ...
    expect(packed).toContain("const PAIR_STORE_SHARD: u32 = 100u;");
