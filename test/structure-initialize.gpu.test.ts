...
interface Manifest {
  readonly embedding: { readonly parameters: Readonly<Record<string, Readonly<Record<string, string>>>> };
  readonly structureModule: { readonly parameters: Readonly<Record<string, Readonly<Record<string, string>>>> };
}
...
describe.skipIf(!enabled)("structure module initialization WebGPU", () => {
  ...
  it("matches official single projection, initial act, and identity affine", async () => {
    ...
    const p = async (map: typeof manifest.embedding.parameters, module: string, name: string) => {
      const tensor = map[module]?.[name]; if (tensor === undefined) throw new Error(`missing ${module}/${name}`);
      return store.tensor(tensor);
    };
    const weights: StructureInitializeWeights = {
