...
interface Manifest { readonly structureModule: { readonly parameters: Readonly<Record<string, Readonly<Record<string, string>>>> } }
...
describe.skipIf(!enabled)("eight-iteration structure core WebGPU", () => {
  ...
  it("matches the final official act and affine state", async () => {
    ...
    const modules = (store.manifest as unknown as Manifest).structureModule.parameters;
    const p = async (module: string, name: string): Promise<Float32Array> => {
    ...
    const ipaWeights: InvariantPointAttentionWeights = {
    ...
    const postAttentionWeights: StructurePostAttentionWeights = {
