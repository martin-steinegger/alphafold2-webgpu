...
export interface ResidueGeometryTables {
  readonly defaultFrames: Float32Array;
  readonly atom14ToGroup: Float32Array;
  readonly atom14Positions: Float32Array;
  readonly atom14Mask: Float32Array;
}

export interface AtomGeometryInput {
...
export interface AtomGeometryResult {
...
export class AtomGeometryGpu {
  ...
  async run(input: AtomGeometryInput): Promise<AtomGeometryResult> {
    ...
    const allocate = (label: string, elements: number, usage = GPUBufferUsage.STORAGE) =>
    ...
    try {
      ...
      const pass = (pipeline: GPUComputePipeline, buffers: readonly AllocatedGpuBuffer[], x: number) => {
