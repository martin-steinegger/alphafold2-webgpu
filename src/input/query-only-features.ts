...
export interface QueryOnlyFeatureTables {
  readonly atom37ToAtom14: Float32Array;
  readonly atom37Mask: Float32Array;
}

export interface QueryOnlyFeatureOptions {
  readonly recycles?: number;
  readonly randomSeed?: number;
  /** Optional exact BERT-MSA codes, one length-L array per recycle. Useful for reproducing an external seeded run. */
  readonly maskedMsaCodes?: readonly Float32Array[];
}
...
function maskedCodes(aatype: Float32Array, recycle: number, seed: number): Float32Array {
...
export function makeQueryOnlyFeatures(
...
): readonly QueryOnlyRecycleFeatures[] {
  ...
  const result: QueryOnlyRecycleFeatures[] = [];
