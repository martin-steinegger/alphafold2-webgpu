import type { TriangleMultiplicationInput, TriangleMultiplicationWeights, TriangleShape } from "../triangle/types.js";
...
export interface TriangleReferenceManifest {
  readonly formatVersion: 1;
  readonly operator: "TriangleMultiplicationOutgoing";
  readonly source: string;
  readonly shape: TriangleShape;
  readonly epsilon: number;
  readonly tensors: Readonly<Record<string, TensorRecord>>;
}

export interface TriangleReferenceBundle {
  readonly input: TriangleMultiplicationInput;
...
export async function loadTriangleReferenceBundle(manifestUrl: URL): Promise<TriangleReferenceBundle> {
  const response = await fetch(manifestUrl);
  if (!response.ok) throw new Error(`failed to load ${manifestUrl}: ${response.status}`);
  const manifest = await response.json() as TriangleReferenceManifest;
